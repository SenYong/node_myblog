        ...
        <article>
           <div class="l_box f_l">
              <div class="topnews">
                  ...
                  <div class="mood">
                      <span class="tutime">{{timestampToTime(sayData.s_time)}}</span>
                  ...
                  <div class="sayPage">
                     <div class="nextpage" v-if="prev">
                        <b>上一条说说:</b>
                        <a @click="SayTo(prev.s_id)" class="pointer">{{prev.s_content}}</a>
                      </div>
                       <div class="nextpage" v-if="next">
                          <b>下一条说说:</b>
                          <a @click="SayTo(next.s_id)" class="pointer">{{next.s_content}}</a>
                      </div>
                  </div>
                  <div class="cont">
                         ...
                         <div class="lItem">
                             ...
                             <div class="lCont">
                                <p class="p1">
                                  ...
                                  <span class="s2">{{timestampToTime(item.sc_time)}}</span>
                                </p>
                         ...
                         <div class="lItem rItem" v-if="item.sc_retime">
                            ...
                            <div class="lCont">
                               <p class="p1">
                                 ...
                                 <span class="s2">{{timestampToTime(item.sc_retime)}}</span>
                               </p>
...
<script type="text/javascript">
  ...
  import { sayDetail, getSayComment, userComment } from '@/api/say';
  ...
  export default{
    ...
    methods: {
      getData(id){
        sayDetail(id).then(res => {
          if(res.code == 0){
            this.sayData = res.data;
            this.prev = res.prev;
            this.next = res.next;
          }
        }).catch(err => {
      ...
      submit(){
        ...
        var sc_pid = this.sc_pid;
        var sc_name = '游客';
        var sc_img = '/public/static/default/'+ ran +'.png';
        var sc_content = this.sc_content;
        this.isDisabled = true;
        userComment(sc_pid, sc_name, sc_img, sc_content).then(res => {
          res.code == 0 ? this.getUserComment(sc_pid) : this.$message.error(res.msg);
          this.sc_content = ''
